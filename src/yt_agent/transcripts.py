...
TAG_RE = re.compile(r"<[^>]+>")
SUBTITLE_LANG_RE = re.compile(r"^[A-Za-z0-9,.*\- ]+$")
...
def _valid_subtitle_languages(value: str) -> bool:
    if not SUBTITLE_LANG_RE.fullmatch(value):
        return False
...
def fetch_subtitle_sidecars(
    ...
    *,
    ...
    if not joined_languages or not _valid_subtitle_languages(joined_languages):
        raise ExternalCommandError("Invalid subtitle language selector.")
    ...
    def _run(write_auto_subs: bool) -> None:
        ...
        try:
            ...
            raise ExternalCommandError("yt-dlp timed out while fetching subtitles.") from exc
