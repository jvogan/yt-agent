...
def test_sanitize_component_suffixes_windows_reserved_names() -> None:
    assert sanitize_component("CON", "fallback") == "CON_"
    assert sanitize_component("com1", "fallback") == "com1_"
...
def test_build_output_template_sanitizes_upload_date(tmp_path: Path) -> None:
    video = VideoInfo(
        video_id="abc123def45",
        title="Demo",
        channel="Channel",
    ...
    output = build_output_template(tmp_path, video)
    assert output.name == "2026 03 10 - Demo [abc123def45].%(ext)s"
