...
def test_fetch_subtitle_sidecars_raises_external_error_on_command_failure(
    ...
    monkeypatch.setattr(
        ...
        lambda args, text, capture_output, check, timeout: subprocess.CompletedProcess(
            args,
            1,
            stdout="",
...
def test_fetch_subtitle_sidecars_rejects_unsafe_language_selector(tmp_path: Path) -> None:
    with pytest.raises(ExternalCommandError, match="Invalid subtitle language selector"):
        fetch_subtitle_sidecars(
            "abc123def45",
            tmp_path / "subs",
            languages=["en;rm -rf"],
...
def test_fetch_subtitle_sidecars_raises_on_timeout(
    tmp_path: Path,
