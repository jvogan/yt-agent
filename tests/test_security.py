...
def test_private_helpers_refuse_symlinked_paths(tmp_path: Path) -> None:
...
@pytest.mark.skipif(sys.platform == "win32", reason="POSIX hardlink semantics")
def test_private_file_refuses_hardlinked_paths(tmp_path: Path) -> None:
    original = tmp_path / "original"
    original.write_text("demo", encoding="utf-8")
    hardlink = tmp_path / "hardlink"
    hardlink.hardlink_to(original)

    with pytest.raises(OSError, match="hardlinked"):
        ensure_private_file(hardlink)
...
@pytest.mark.skipif(sys.platform == "win32", reason="POSIX symlink semantics")
def test_atomic_write_text_refuses_existing_symlink(tmp_path: Path) -> None:
