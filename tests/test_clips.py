...
def test_run_raises_external_command_error_on_timeout(monkeypatch) -> None:
    def fake_run(args, text, capture_output, check, timeout):
...
def test_extract_resolved_clip_prefers_remote_when_requested(
    ...
    output_template = Path(recorded[0][7])
    assert output_template.parent == extraction.output_path.parent
    assert output_template.name.startswith(f".{extraction.output_path.stem}.")
    assert output_template.name.endswith(".%(ext)s")
    assert recorded[0][8] == "https://www.youtube.com/watch?v=abc123def45"


def test_extract_resolved_clip_rejects_local_media_outside_download_root(
    settings,
    monkeypatch,
    tmp_path: Path,
) -> None:
...
def test_extract_resolved_clip_rejects_symlink_clip_output(
    settings,
    monkeypatch,
    tmp_path: Path,
) -> None:
...
def test_extract_resolved_clip_revalidates_remote_url(settings, monkeypatch) -> None:
    info = VideoInfo(
        video_id="abc123def45",
        title="Demo Video",
        channel="Channel",
        ...
        duration_seconds=120,
        extractor_key="youtube",
        webpage_url="https://example.com/watch?v=abc123def45",
    )
    calls: list[list[str]] = []
...
def test_extract_resolved_clip_rehardens_clip_directory(
    ...
    assert created_dirs == [extraction.output_path.parent, extraction.output_path.parent]
